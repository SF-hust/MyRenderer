...
template<size_t N, typename T>
inline T VecNT_length_square(const Vector<N, T>& a) {
	T sum = 0;
...
template<size_t N, typename T>
inline T VecNT_length(const Vector<N, T>& a) {
	return sqrt(VecNT_length_square(a));
}

template<size_t N>
inline float VecNT_length(const Vector<N, float>& a) {
	return sqrtf(VecNT_length_square(a));
}

// 向量规格化 normalize
template<size_t N, typename T>
inline Vector<N, T> VecNT_normalize(const Vector<N, T>& a) {
	return a / VecNT_length(a);
}

// 矢量点乘
template<size_t N, typename T>
inline T VecNT_dot(const Vector<N, T>& a, const Vector<N, T>& b) {
	T sum = 0;
...
// 二维矢量叉乘
template<typename T>
inline T VecNT_cross(const Vector<2, T>& a, const Vector<2, T>& b) {
	return a.x * b.y - a.y * b.x;
}

// 三维矢量叉乘
template<typename T>
inline Vector<3, T> VecNT_cross(const Vector<3, T>& a, const Vector<3, T>& b) {
	return Vector<3, T>(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
}

// 四维矢量叉乘：前三维叉乘，后一位保留
template<typename T>
inline Vector<4, T> VecNT_cross(const Vector<4, T>& a, const Vector<4, T>& b) {
	return Vector<4, T>(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x, a.w);
}

// 线性插值
template<size_t N, typename T>
inline Vector<N, T> VecNT_lerp(const Vector<N, T>& a, const Vector<N, T>& b, float t) {
	return a + (b - a) * t;
}

// 各个元素取最大值
template<size_t N, typename T>
inline Vector<N, T> VecNT_max(const Vector<N, T>& a, const Vector<N, T>& b) {
	Vector<N, T> c;
...
// 各个元素取最小值
template<size_t N, typename T>
inline Vector<N, T> VecNT_min(const Vector<N, T>& a, const Vector<N, T>& b) {
	Vector<N, T> c;
	for (size_t i = 0; i < N; i++) c[i] = (a[i] < b[i]) ? a[i] : b[i];
	return c;
}
...
template<size_t N, typename T>
inline Vector<N, T> VecNT_between(const Vector<N, T>& minx, const Vector<N, T>& maxx, const Vector<N, T>& x) {
	return VecNT_min(VecNT_max(minx, x), maxx);
}

// 判断矢量是否接近
template<size_t N, typename T>
inline bool VecNT_near(const Vector<N, T>& a, const Vector<N, T>& b, T dist) {
	return (VecNT_length_square(a - b) <= dist);
}

// 判断两个单精度矢量是否近似
template<size_t N>
inline bool VecNT_near_equal(const Vector<N, float>& a, const Vector<N, float>& b, float e = 0.0001) {
	return VecNT_near(a, b, e);
}

// 判断两个双精度矢量是否近似
template<size_t N>
inline bool VecNT_near_equal(const Vector<N, double>& a, const Vector<N, double>& b, double e = 0.0000001) {
	return VecNT_near(a, b, e);
}

// 矢量值元素范围裁剪
template<size_t N, typename T>
inline Vector<N, T> VecNT_clamp(const Vector<N, T>& a, T minx = 0, T maxx = 1) {
	Vector<N, T> b;
...
// 输出成字符串
template<size_t N, typename T>
inline std::string VecNT_repr(const Vector<N, T>& a) {
	std::stringstream ss;
	ss << a;
	return ss.str();
}
...
inline Matrix<ROW, NEWCOL, T> operator * (const Matrix<ROW, COL, T>& a, const Matrix<COL, NEWCOL, T>& b) {
	...
	for (size_t j = 0; j < ROW; j++) {
		for (size_t i = 0; i < NEWCOL; i++) {
			out.m[j][i] = VecNT_dot(a.Row(j), b.Col(i));
		}
	}
	return out;
}
...
inline Matrix<N, N, T> matrix_invert(const Matrix<N, N, T>& m) {
	...
	T det = VecNT_dot(m.Row(0), ret.Col(0));
	return ret / det;
}
...
inline static Mat4x4f matrix_set_rotate(float x, float y, float z, float theta) {
	...
	float w = qcos;
	Vec3f vec = VecNT_normalize(Vec3f(x, y, z));
	x = vec.x * qsin;
	y = vec.y * qsin;
	z = vec.z * qsin;
	Mat4x4f m;
...
inline static Mat4x4f matrix_set_lookat(const Vec3f& eye, const Vec3f& at, const Vec3f& up) {
	Vec3f zaxis = VecNT_normalize(at - eye);
	Vec3f xaxis = VecNT_normalize(VecNT_cross(up, zaxis));
	Vec3f yaxis = VecNT_cross(zaxis, xaxis);
	Mat4x4f m;
	m.SetCol(0, Vec4f(xaxis.x, xaxis.y, xaxis.z, -VecNT_dot(eye, xaxis)));
	m.SetCol(1, Vec4f(yaxis.x, yaxis.y, yaxis.z, -VecNT_dot(eye, yaxis)));
	m.SetCol(2, Vec4f(zaxis.x, zaxis.y, zaxis.z, -VecNT_dot(eye, zaxis)));
